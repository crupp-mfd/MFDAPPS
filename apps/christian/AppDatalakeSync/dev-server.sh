...
export PYTHONPATH="$ROOT_DIR:$ROOT_DIR/packages/sparepart-shared/src:$ROOT_DIR/apps/christian/AppDatalakeSync/src:${PYTHONPATH:-}"
...
if [[ ${#extra[@]} -eq 0 ]]; then
  exec uvicorn app_datalake_sync.main:app --reload "$@"
fi

exec uvicorn app_datalake_sync.main:app --reload "${extra[@]}" "$@"
